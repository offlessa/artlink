...
Para compreender o cenário atual e identificar oportunidades de melhoria, foram analisadas três plataformas de referência com propostas distintas de apoio ao artesanato: Instagram, Etsy e ArtFinder.

O \textbf{Instagram} é a rede social generalista mais utilizada por artistas e artesãos para divulgar seu trabalho atualmente \cite{instagram2024}. A plataforma oferece \textit{feed} de publicações, sistema de seguidores, curtidas e comentários já consolidados, com grande alcance de público, além de um recurso básico de coautoria em publicações (\textit{Collabs}). Contudo, por não ter sido projetada para o universo criativo, carece de ferramentas de organização de portfólio, não oferece um fluxo formal de convite, aceite e recusa de colaboração como o implementado no Artlink, e sua audiência é genérica, dispersa entre todo tipo de conteúdo, sem o mesmo foco no universo artístico e artesanal.
...
O \textbf{Artlink} foi desenvolvido para preencher as lacunas identificadas nessas plataformas, combinando elementos de rede social com ferramentas de divulgação artística. A Tabela~\ref{tab:comparativo} apresenta um comparativo entre as quatro plataformas.
...
\hline
\textbf{Funcionalidades} & \textbf{Instagram} & \textbf{Etsy} & \textbf{ArtFinder} & \textbf{Artlink} \\ \hline
\textit{Feed} social e publicações  & Sim & Não  & Não   & Sim   \\ \hline
Sistema de seguidores               & Sim & Não  & Não   & Sim   \\ \hline
Mensagens diretas entre usuários    & Sim & Parcial & Não  & Sim  \\ \hline
Colaboração entre artistas          & Parcial & Não  & Não   & Sim   \\ \hline
Catálogos e coleções de obras       & Não & Parcial & Sim  & Sim  \\ \hline
Curtidas e comentários              & Sim & Não  & Parcial & Sim  \\ \hline
Notificações de interações          & Sim & Sim  & Parcial & Sim  \\ \hline
Busca por \textit{hashtags}         & Sim & Parcial & Não  & Sim  \\ \hline
Gestão de estoque e vendas          & Não & Sim  & Sim   & Não   \\ \hline
\end{tabular}
\caption*{Fonte: Elaborado pelo autor (2025)}
\end{table}

O comparativo evidencia que o Artlink se diferencia tanto das redes sociais generalistas, como o Instagram, que carecem de ferramentas de portfólio e de um fluxo estruturado de colaboração entre criadores, quanto das plataformas de comércio eletrônico, como Etsy e ArtFinder, que carecem de um componente social completo. Em contrapartida, funcionalidades de \textit{e-commerce}, presentes nas demais plataformas analisadas, não fazem parte do escopo do Artlink. Essa escolha reflete o objetivo central do sistema: antes de comercializar, conectar.
...
\begin{itemize}
    ...
    \item \textbf{Implementação:} O desenvolvimento foi realizado no \textit{Visual Studio Code} (VS Code), utilizando React no \textit{frontend} e Node.js com Express no \textit{backend}, dentro do ecossistema JavaScript/TypeScript em ambas as camadas da aplicação. A organização modular do código garantiu escalabilidade e facilidade de manutenção.
...
\begin{figure}[!ht]
\caption{Modelo relacional completo do banco de dados do Artlink}
\centering
\includegraphics[width=\textwidth]{Imagens/diagrama_completo_banco_dados.png}\\[6pt]
{\small Cinza = tabela de junção (chave composta); branco = entidade com chave estrangeira; cor = entidade principal. Fonte: Elaborado pelo autor (2025)}
...
\begin{itemize}
    ...
    \item Implementação de um sistema de denúncia e moderação de conteúdo, combinando revisão manual por administradores com análise automática de imagens via inteligência artificial (por exemplo, Amazon Rekognition ou WebPurify), a fim de impedir a publicação de conteúdo impróprio.
\end{itemize}
